...
Oltre a vedere le coppie di termini più frequenti è possibile andare a studiare le parole che possiedono maggior correlazione, perchè compaiono nella stessa situazione ma non sono adiacenti.

```{r , echo=FALSE, collapse=TRUE, warning=FALSE,message=FALSE}
word_cors <- ds_singleWord %>% filter(word != "ah", word != "ha") %>% #filter(Season == 1) %>% 
  group_by(word) %>%
  filter(n() >= 90) %>%
